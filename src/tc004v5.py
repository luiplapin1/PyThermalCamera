...
def draw_measurement_hud(image, thdata):
    ...
    # Background for measurement HUD
...
def update_measurement_points_scale(new_scale):
    """Update screen coordinates of points when scale changes"""
...
def test_camera_access(device_num):
    """Test different methods to access camera"""
    methods = [
...
def main():
    ...
    if cap is None:
        ...
        print("Suggestions:")
        print("- Check that camera is connected")
    ...
    print("  Right click: Remove nearby point")
    print("ESC: Exit")
    print("-" * 50)
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if frame_height >= 384:  # Complete frame with thermal data
            ...
            if thdata.shape[0] >= 96 and thdata.shape[1] >= 128:
                ...
                try:
                    ...
                    print(f"Error calculating center temperature: {e}")
            ...
            if colormap == 10:  # Rainbow inverted
        ...
        if keyPress == ord('a'):
            ...
            try:
                cv2.imwrite(f"TC001{now}.png", heatmap)
                print(f"Capture saved: TC001{now}.png")
                # Also save measurement points data
                if measurement_points:
                    with open(f"TC001{now}_measurements.txt", "w") as f:
                        f.write(f"Capture taken: {snaptime}\n")
                        f.write(f"Average temperature: {avgtemp}°C\n")
                        f.write(f"Center temperature: {temp}°C\n")
                    ...
                    print(f"Measurement data saved: TC001{now}_measurements.txt")
            except Exception as e:
